...
Fusion::Fusion(const std::vector<DistanceSensor*>& distance_sensors, std::function<double()> velocity_sensor) :
_distance_sensors(distance_sensors),
...
//    std::transform(_distance_sensors.begin(), _distance_sensors.end(), distance_measurements.begin(), [](DistanceSensor* sensor)
//    {
//        return sensor->getDistanceToClosestObject();
//    });
    
    std::sort(distance_measurements.begin(), distance_measurements.end(), [](const auto &lhs, const auto &rhs)
    {
